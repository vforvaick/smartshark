...
export default function DisplayFilter({ onApplyFilter, loading }: DisplayFilterProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-2 bg-white p-2 border rounded shadow-sm">
      <span className="text-xs font-semibold text-gray-500 uppercase px-1">Filter:</span>
      <input
        type="text"
        placeholder="e.g. ip.src == 192.168.1.1 or tcp.flags.syn == 1"
        value={filterText}
        onChange={(e) => setFilterText(e.target.value)}
        className="flex-1 rounded border border-gray-300 px-3 py-1 text-sm font-mono focus:border-blue-500 focus:outline-none"
      />
      <button
        type="submit"
        disabled={loading}
